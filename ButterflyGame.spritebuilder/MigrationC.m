    ...
    if ([userDefaults objectForKey:mHighestJourneyCStopUnlocked]) {
        ...
        if (self.unlockJourney) {
            ...
            if (self.highestPlayableStop < 4) {
                ...
                [userDefaults setInteger:self.highestPlayableStop forKey:mHighestJourneyCStopUnlocked];
                [userDefaults synchronize];
            } else {
                if (self.highestPlayableStop == 4) {
                    // unlock the next journey
                    [userDefaults setInteger:4 forKey:mHighestJourneyUnlocked];
                    [userDefaults synchronize];
                }
            }
        ...
        // set the default
        [userDefaults setInteger:1 forKey:mHighestJourneyCStopUnlocked];
