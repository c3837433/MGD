...
-(void) onEnter {
    ...
    self.highestPlayableStop = [GameData sharedGameData].gameActivePlayer.highestCStop;
    ...
    if (self.unlockJourney) {
        ...
        if (self.highestPlayableStop < 4) {
            percentComplete = self.highestPlayableStop * 25;
...
-(void)shouldPlaySelectedLevel {
    ...
    [Utility shouldPlaySelectedLevelStop:selectedStop andHighestStop:self.highestPlayableStop forJourney:@"C" withPlayer:[GameData sharedGameData].gameActivePlayer];
}
