...
-(void) onEnter {
    ...
    if (self.sessionThroughGameCenter) {
        ...
        self.highestPlayableStop = [GameData sharedGameData].gameCenterPlayer.highestCStop;
        ...
        if (self.unlockJourney) {
            ...
            if (self.highestPlayableStop < 4) {
                ...
                [GameData sharedGameData].gameCenterPlayer.highestCStop ++;
        ...
        self.highestPlayableStop = [GameData sharedGameData].gameLocalPlayer.highestCStop;
        selectedStop = self.highestPlayableStop;
        if (self.unlockJourney) {
            // need to unlock another stop on the map
            if (self.highestPlayableStop < 4) {
                ...
                self.highestPlayableStop ++;
                [[GameData sharedGameData] save];
                [GameData sharedGameData].gameLocalPlayer.highestCStop ++;
            } else if (self.highestPlayableStop == 4) {
                // unlock the next journey
...
-(void)getTopScoreForStop: (NSInteger)stop andPlayer:(Player*)player {
    ...
    for (GameScore* score in scores) {
        ...
        if (([score.gameJourney isEqualToString:@"C"]) && (score.gameStop == stop) && ([score.gamePlayer isEqual:player])) {
...
-(void)shouldPlaySelectedLevel {
    ...
    [Utility shouldPlaySelectedLevelStop:selectedStop andHighestStop:self.highestPlayableStop forJourney:@"C" withPlayer:self.player andConnection:self.sessionThroughGameCenter];
}
