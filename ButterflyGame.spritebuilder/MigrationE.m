...
-(void) onEnter {
    ...
    self.highestPlayableStop = [GameData sharedGameData].gameActivePlayer.highestEStop;
    ...
    if (self.unlockJourney) {
        ...
        if (self.unlockJourney) {
            percentComplete = self.highestPlayableStop * 20;
            // need to unlock another stop on the map
...
-(void)shouldPlaySelectedLevel {
    ...
    [Utility shouldPlaySelectedLevelStop:selectedStop andHighestStop:self.highestPlayableStop forJourney:@"E" withPlayer:[GameData sharedGameData].gameActivePlayer];
}
